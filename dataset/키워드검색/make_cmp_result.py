...
data = pd.read_csv('cmp_info_data.csv', low_memory=False)
...
def get_recommendations(title, cosine_sim=cosine_sim):
    # 선택한 회사정보의 이름으로부터 인덱스를 받아온다.
    idx = title_to_index[title]

    # 해당 회사정보와 모든 메세지와의 유사도를 가져온다.
    sim_scores = list(enumerate(cosine_sim[idx]))

    # 유사도에 따라 회사정보들을 정렬한다.
    sim_scores = sorted(sim_scores, key=lambda x: x[1], reverse=True)

    # 가장 유사한 10개의 회사정보를 받아온다.
    sim_scores = sim_scores[1:11]
    print(sim_scores)

    # 가장 유사한 10개의 인덱스를 얻는다.
    movie_indices = [idx[0] for idx in sim_scores]

    # 가장 유사한 10개의 회사이름을 리턴한다.
    return data['title'].iloc[movie_indices], data['overview'].iloc[movie_indices]
